...
public class MemoryExtractorBlockEntityRenderer implements BlockEntityRenderer<MemoryExtractorBlockEntity> {

    private final ItemRenderer itemRenderer;

    public MemoryExtractorBlockEntityRenderer(BlockEntityRendererFactory.Context context) {
        this.itemRenderer = context.getItemRenderer();
    }

    @Override
    public void render(MemoryExtractorBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
        ...
        matrices.push();
        Direction direction = entity.getCachedState().get(MemoryExtractorBlock.FACING);
        Direction opposite = direction.getOpposite();
        boolean hasUp = entity.getCachedState().get(MemoryExtractorBlock.UP);
        if (direction.getHorizontal() == -1) {
            ...
            boolean hasDown = entity.getCachedState().get(MemoryExtractorBlock.DOWN);
